...
CmdClaw configures per-generation MCP access through OpenCode's runtime MCP API before the first prompt, then uses OpenCode's MCP status API to verify which servers connected. The runtime reconciles the allowlist dynamically: stale connected servers are disconnected and every allowlisted server is upserted and connected via `mcp.add`, with a bounded per-server wait so one unreachable server cannot stall generation start. Dynamic MCP state is process-local: if OpenCode restarts mid-conversation it comes back with no MCP servers until the next generation's reconcile re-adds them, which the per-generation reconciliation requirement below already guarantees.

> **Amended 2026-06-10.** This consequence originally required writing the allowlist into the sandbox-local OpenCode config and restarting OpenCode on config changes, "because dynamic MCP add/connect calls do not make new tools reliably visible to the prompt loop." That premise no longer holds: OpenCode ≥ 1.16 exposes a first-class `mcp.add` that upserts config and connects, and tools added dynamically were verified visible and callable by the prompt loop of an existing session on 1.16.2 (the pinned sandbox version) and 1.17.0. The restart approach cost 3–5s per generation and discarded the sandbox boot warmup, so the runtime now reconciles dynamically.
