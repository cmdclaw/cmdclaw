...
CmdClaw archives Daytona sandboxes for a short, bounded period after platform-suspect runtime failures so operators can recover the filesystem state that existed when the failure happened. The first eligible failures are `runtime_no_progress_after_prompt` and runtime-turn `runtime_error`. The archive is evidence for debugging, not an active runtime for continuing the Generation.
