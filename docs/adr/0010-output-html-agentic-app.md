...
Bap will treat a sandbox file named exactly `output.html` as a self-contained **Agentic-App** when support is enabled by the chat surface. The runtime will auto-collect `output.html` even when the assistant does not mention it, store it through the existing sandbox file path, and the web app will render it in a collapsible chat-side panel by fetching authenticated HTML and passing it to a sandboxed iframe as `srcDoc`.
...
`output.html` must be a single self-contained document for v1. Relative asset bundles are not supported until Bap introduces a broader artifact serving model.
