...
Bap supports multiple Connected Accounts for the same User and Integration Type, and each Tool Invocation targets one Connected Account. We decided to introduce Connected Identity as the internal grouping that owns the user-facing Account Label, while each `integration` row remains one credential-bearing Connected Account for one Integration Type under that Connected Identity. The previous one-row-per-User-and-Integration-Type model will be replaced, and Account Labels are no longer owned independently by each Integration Type.
...
Email grouping must not violate the invariant that a Connected Identity has at most one Connected Account per Integration Type. When multiple accounts of the same Integration Type share the same email but differ by workspace, tenant, or org, Bap creates separate Account Labels with contextual suffixes rather than grouping them under one label.
