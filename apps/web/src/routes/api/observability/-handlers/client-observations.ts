 ...
 *
 * Authenticated client observation/telemetry intake. Authorization is enforced
 * inside the handler (Better Auth session + active workspace access + per-row
 * conversation/generation ownership checks). Uses standard `Request`/`Response`
 * only. Durable rate limiting and dedupe are backed by Redis so correctness does
 * not depend on in-memory state.
 */
