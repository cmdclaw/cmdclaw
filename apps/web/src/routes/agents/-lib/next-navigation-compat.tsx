 ...
 *
 * These components (`coworker-editor-page`, `coworker-info-page`) build raw string hrefs and
 * drive navigation with `router.push(href)` / `router.replace(href)`, plus read the current
 * pathname and search params. Rather than rewrite ~5000 lines of stable product logic during
 * the Big Bang framework migration, this module maps that exact API onto TanStack Router's
 * `useNavigate` / `useRouterState`. It is route-local (lives under the agents area) and is the
 * only place those components touch routing.
 *
 ...
 * shared editor component can resolve its `:id` segment regardless of which agents route
 * (edit / runs / runs detail) is currently active.
 */
