...
function getMagicLinkLogoUrl(): string {
...
export function buildMagicLinkEmailPayload(
  signInUrl: string,
  email: string,
): {
  html: string;
  ...
  const signInPageUrl = new URL(signInUrl);
  const safeEmail = escapeHtml(email);
  const safeSignInUrl = escapeHtml(signInUrl);
  const safeLogoUrl = escapeHtml(getMagicLinkLogoUrl());

  return {
    text: `CMDCLAW SIGN-IN LINK

A sign-in link was requested for ${email}.

Sign in securely: ${signInUrl}

For your safety:
- This link opens on ${signInPageUrl.hostname.replace(/^www\./, "")}
        ...
        <table role="presentation" style="max-width: 480px; width: 100%; border-collapse: collapse; background-color: #ffffff; border: 2px solid #B33A3A; border-radius: 12px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 0 40px;">
              <h1 style="margin: 0 0 16px 0; font-size: 22px; font-weight: 600; letter-spacing: -0.02em; color: #1c1917; text-align: center; line-height: 1.3;">Sign in to CmdClaw</h1>
              <p style="margin: 0 0 8px 0; font-size: 15px; color: #57534e; line-height: 1.6; text-align: center;">A sign-in link was requested for<br><strong style="color: #1c1917;">${safeEmail}</strong></p>
              ...
              <table role="presentation" style="margin: 0 auto 32px auto; border-collapse: collapse;">
                <tr>
                  <td align="center" style="background-color: #18181b; border-radius: 8px;">
                    <a href="${safeSignInUrl}" style="display: inline-block; padding: 14px 36px; background-color: #18181b; color: #ffffff; text-decoration: none; font-size: 14px; font-weight: 600; letter-spacing: 0.02em; border-radius: 8px;">Sign in</a>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 0 40px 32px 40px;">
              ...
              <p style="margin: 0; font-size: 12px; line-height: 1.5; word-break: break-all;">
                <a href="${safeSignInUrl}" style="color: #B33A3A; text-decoration: underline;">${safeSignInUrl}</a>
              </p>
            </td>
          </tr>
          <!-- Footer -->
