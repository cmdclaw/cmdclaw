...
test.describe("@live chat question", () => {
  test.skip(!liveEnabled, "Set E2E_LIVE=1 to run live Playwright tests");
  test.use({ storageState: storageStatePath });

  test("submits selected question option and model uses it", async ({ page, liveChatModel }) => {
    ...
    const betaOption = page.getByTestId("question-option-0-Beta").first();
    ...
    await expect
      .poll(async () => assistantMessages.count(), {
        ...
        message: "Assistant did not produce a persisted message after question answer",
      })
      .toBeGreaterThan(initialAssistantCount);

    const assistantBubble = page.getByTestId("chat-bubble-assistant").last();
    await expect(assistantBubble).toContainText("SELECTED=Beta", { timeout: responseTimeoutMs });
  });
});
