...
export default function AdminUsagePage() {
  ...
  const { chartData, chartKeys, chartColors } = useMemo(() => {
    ...
    if (groupBy === "workspace") {
      for (const entry of (usageQuery.data?.dailyByWorkspace ?? []) as DailyByWorkspaceEntry[]) {
  ...
  const workspaceBreakdown = useMemo(() => {
    const rows = (usageQuery.data?.workspaceBreakdown ?? []) as WorkspaceBreakdownEntry[];
    const grandTotal = rows.reduce((sum, row) => sum + row.totalTokens, 0);
    return rows.map((row) => ({
  ...
  const isLoading = workspacesQuery.isLoading || (usageQuery.isLoading && !usageQuery.data);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-card rounded-lg border p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <Select value={groupBy} onValueChange={handleGroupByChange}>
            ...
            <SelectContent>
              {workspaceId === "all" ? <SelectItem value="workspace">Workspace</SelectItem> : null}
        ...
        {chartData.length === 0 ? (
          <div className="text-muted-foreground flex h-[380px] items-center justify-center text-sm">
            No usage data {workspaceId !== "all" ? "for this workspace " : ""}in the last 30 days.
          </div>
        ) : (
      ...
      {workspaceId === "all" && workspaceBreakdown.length > 0 ? (
        <section className="bg-card rounded-lg border p-5">
          <div className="mb-4">
              ...
              {coworkerBreakdown.length === 0 ? (
                <tr className="border-t">
                  <td colSpan={7} className="text-muted-foreground px-3 py-8 text-center">
                    No usage data {workspaceId !== "all" ? "for this workspace " : ""}in the last 30
                    days.
                  </td>
                </tr>
              ) : (
