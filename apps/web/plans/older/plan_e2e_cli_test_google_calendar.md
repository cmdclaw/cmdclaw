# Plan: E2E CLI Test for google_calendar

## Goal
Create a live end-to-end CLI test for the google_calendar integration that validates the full path from `bun run chat` prompt execution to externally verifiable read results from the provider API.
...
- Do not add write-operation checks for google_calendar.
- Do not add approval-workflow assertions for google_calendar.
...
- Before coding the E2E test, first run `bun run chat` manually against google_calendar to understand real skill behavior and output shape.
- If google_calendar is not connected for the test user, stop implementation and ask the user to connect it before proceeding.

## Notes
This is a live integration test and requires the test user to already be connected to google_calendar.
