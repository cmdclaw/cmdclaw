...
## Testing workflow
-  After implementing a feature, test it with `bun run bap -- chat` when possible.
-  Example chat validation command: `bun run bap -- chat --message "what's my latest email on gmail?" --model openai/gpt-5.4`
-  After implementing a coworker feature, or when chat is not enough to cover the user flow, test it with `bun run bap -- coworker`.
-  Example coworker validation command: `bun run bap -- coworker create --name "Email Check" --trigger manual --prompt "check my latest email every hour" --auto-approve`
-  If `bun run bap -- chat` is not sufficient to validate the change, clearly report that limitation. If applicable, say how you would change `bap chat` to support testing this feature.
...
-  If it does not log in, make sure the dev server was started with `BAP_DEV_AUTO_LOGIN=1`.
