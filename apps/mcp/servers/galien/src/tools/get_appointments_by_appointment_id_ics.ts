...
export default async function getAppointmentsByAppointmentIdIcs(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/appointments/{appointmentId}/ics", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
