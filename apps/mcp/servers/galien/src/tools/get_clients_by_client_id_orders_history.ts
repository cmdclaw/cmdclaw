...
export default async function getClientsByClientIdOrdersHistory(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/clients/{clientId}/orders-history", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
