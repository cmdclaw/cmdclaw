...
export default async function getDestructionCertificatesByDestructionCertificateIdGenerate(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/destructionCertificates/{destructionCertificateId}/generate", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
