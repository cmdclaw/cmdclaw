...
export default async function getClientsByClientIdReportsNumber(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/clients/{clientId}/reports-number", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
