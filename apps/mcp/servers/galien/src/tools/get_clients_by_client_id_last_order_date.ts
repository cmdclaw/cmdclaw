...
export default async function getClientsByClientIdLastOrderDate(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/clients/{clientId}/last-order-date", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
