...
export default async function getClientsByClientIdNextAppointment(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/clients/{clientId}/next-appointment", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
