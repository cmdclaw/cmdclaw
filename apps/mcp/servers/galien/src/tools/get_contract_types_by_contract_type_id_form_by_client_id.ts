...
export default async function getContractTypesByContractTypeIdFormByClientId(params: InferSchema<typeof schema>) {
  const result = await requestGalienGet("/api/v1/contract-types/{contractTypeId}/form/{clientId}", params as Record<string, string | number | boolean | Array<string | number | boolean> | undefined>);
