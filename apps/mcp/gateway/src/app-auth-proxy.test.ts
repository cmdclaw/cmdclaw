...
describe("shouldProxyAppAuthFlowPath", () => {
  ...
  it("does not reintroduce the old Bap MCP slug as an app proxy path", () => {
    expect(shouldProxyAppAuthFlowPath("/bap")).toBe(false);
    expect(shouldProxyAppAuthFlowPath("/bap/authorize")).toBe(false);
  });
});
