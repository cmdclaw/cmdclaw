...
vi.mock("@/server/services/coworker-service", () => ({
  triggerCoworkerRun: triggerCoworkerRunMock,
}));

vi.mock("@/server/services/coworker-scheduler", () => ({
...
vi.mock("@/server/services/coworker-builder-service", async () => {
  const actual = await vi.importActual<typeof import("@/server/services/coworker-builder-service")>(
    "@/server/services/coworker-builder-service",
  );
  return {
    ...actual,
